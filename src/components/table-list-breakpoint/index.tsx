...
const Component = (props: Props) => {
  ...
  const captionData =
    typeof caption == 'function' ? caption(listView) : caption;
  ...
  return (
    <>
      {!listView && (
        <table className={`table-list-breakpoint table-view ${className}`}>
          {captionData && <caption>{captionData}</caption>}

          {headData && (
            <thead>
              <tr>
                {theadData.map((head, key) => (
                  <th key={key} className={`${tableFieldClass} ${head.name}`}>
                    {head.value}
                  </th>
                ))}
              </tr>
          ...
          <tbody>
            {tdData.map((row, trKey) => (
              <tr key={trKey}>
                {row.map((item, tdKey) => (
                  <td key={tdKey} className={`${tableFieldClass} ${item.name}`}>
      ...
      {listView && (
        <div className={`table-list-breakpoint list-view ${className}`}>
          {captionData && <h2 className="list-title">{captionData}</h2>}
                ...
                {row.map((item, itemKey) => (
                  ...
                  >
                    {headData && headData[item.name] && (
                      <div className="label">{headData[item.name]}</div>
                    )}
