          ...
          >
            ...
            {children.length > 0 && (
              <div className="children">
                <NestedOptions options={children} onChange={onChange} />
              </div>
            )}
          </div>
        )
...
const mapOptionsState = (values: Option[], lastId = 0): Option[] => {
  ...
  return values.map((value) => {
    ...
    return {
      ...
      options: value.options ? mapOptionsState(value.options, id) : undefined,
    };
  });
};
...
const selectOptionsById = (
  ...
  return values.map((value) => {
    return {
      ...
      options: value.options ? selectOptionsById(value.options, id) : undefined,
    };
  });
};
  ...
  return (
    <div className={className}>
      ...
      >
        {!customSelected && (
          <>
            {selected && <div className="value">{selected.data.label}</div>}

            {!selected && <div className="value">{placeholder}</div>}
          </>
        )}
